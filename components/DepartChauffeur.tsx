...
const DepartChauffeur: React.FC<DepartChauffeurProps> = ({ records, onSave }) => {
    ...
    return (
        <div className="h-full flex flex-col">
            <h2 className="text-xl font-bold text-gray-700 mb-4">{t('adminPanel.departChauffeur.title')}</h2>
            ...
            {/* Modal for Departure Info */}
            {isModalOpen && selectedRecord && (
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
                    <div className="bg-white rounded-lg shadow-2xl w-full max-w-lg max-h-[90vh] flex flex-col">
                         <div className="p-6 border-b">
                            <h3 className="text-lg font-bold text-gray-800">{t('adminPanel.departChauffeur.modalTitle')}</h3>
                             <p className="text-sm text-gray-600">{t('adminPanel.departChauffeur.driverInfo', { name: selectedRecord.driver.name, time: selectedRecord.checkinTime.toLocaleTimeString('es-ES') })}</p>
                        </div>
                        <form onSubmit={handleSubmit} className="flex-grow overflow-y-auto">
                           <div className="p-6">
                                <label htmlFor="notes" className="block text-sm font-medium text-gray-700">{t('adminPanel.departChauffeur.notesLabel')}</label>
                                <textarea 
                                    name="notes" 
                                    id="notes" 
                                    value={notes} 
